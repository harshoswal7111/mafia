...
<div class="container mx-auto py-6 px-4">
    <div class="text-center mb-10">
        <h1 class="text-3xl font-bold text-red-900 mb-4">Game Lobby</h1>
    </div>
    ...
    <div class="bg-white p-5 rounded-lg shadow-md mb-6">
        ...
        <div class="bg-white shadow-lg rounded-xl p-6 mb-6">
            ...
            <div id="playerList" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            .then(data => {
                ...
                if (playerListContainer) {
                    ...
                    for (const [playerId, player] of Object.entries(data.players)) {
                        ...
                        // Generate proper initials for the avatar
                        let initials = '';
                        if (!player.photoPath) {
                        ...
                        playerListHTML += `
                            <div class="flex items-center justify-between mb-4 bg-white p-4 rounded-lg shadow">
                                ${player.photoPath ? 
                                    `<div class="w-10 h-10 rounded-full overflow-hidden bg-gray-200 mr-3">
                                        <img src="${player.photoPath}" alt="Profile" class="w-full h-full object-cover">
                                    </div>` : 
                                    `<div class="w-10 h-10 rounded-full flex items-center justify-center bg-blue-600 text-white font-bold mr-3">
                                        ${initials}
                                    </div>`}
                                
                                <span class="font-medium ${isHost ? 'text-red-600' : ''}">
                                    ${player.name}
                                    ${isHost ? '<span class="text-xs">(Host)</span>' : ''}
                                </span>
                                
                                ${isHost && !isCurrentPlayer && <?php echo $isHost ? 'true' : 'false'; ?> ? `
                                    <form action="index.php" method="get" class="ml-4">
                                        <input type="hidden" name="action" value="remove_player">
                                        <input type="hidden" name="code" value="<?php echo $gameCode; ?>">
                                        <input type="hidden" name="playerId" value="${playerId}">
                                        <button type="submit" class="text-gray-500 hover:text-red-600">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
